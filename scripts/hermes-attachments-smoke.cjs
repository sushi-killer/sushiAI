...
(async () => {
  ...
  try {
    ...
    await fs.writeFile(
      home + "/skills/synthetic-command/SKILL.md",
    ...
    const fileInput = {
      agentId: "default",
      path: home + "/synthetic-editor.txt",
    };
    const files = (action, input) =>
      p.operations.get("addons.files." + action)(input);
    const listing = await files("list", { agentId: "default", path: home });
    assert.ok(
      listing.entries.some((entry) => entry.name === "synthetic-editor.txt"),
    );
    ...
    await assert.rejects(
      files("save", {
        ...encodedInput,
        revision: encodingPreview.revision,
        content: "Do not overwrite",
      }),
    );
    ...
    await files("save", {
      ...encodedInput,
      revision: utf8Preview.revision,
      content: "Edited \uFFFD character",
    });
    assert.equal(
      await fs.readFile(encodedPath, "utf8"),
      "Edited \uFFFD character",
    );
    const repo = home + "/synthetic-repo";
    await fs.mkdir(repo);
    ...
    const gitRead = (action, extra = {}) =>
      p.operations.get("addons.git." + action)({
        agentId: "default",
        path: repo,
        ...extra,
      });
    const gitStatus = await gitRead("status");
    assert.equal(gitStatus.branch, "main");
    assert.equal(gitStatus.files.length, 2);
    const diff = await gitRead("diff", { file: "note.txt" });
    assert.match(diff.diff, /-Original line/);
    assert.match(diff.diff, /\+Changed line/);
    assert.match(
      (await gitRead("diff", { file: "new.txt" })).diff,
      /\+New file/,
    );
    const chat = await p.create({
      agentId: "default",
      title: "Synthetic attachments",
    });
    ...
    assert.ok(
      skillCatalog.commands.some(
        (command) => command.name === "/synthetic-command",
      ),
    );
    await p.operations.get("conversations.runSkill")({
      ...input,
      command: "/synthetic-command",
      arguments: "Synthetic user task",
    });
    const skillDeadline = Date.now() + 30000;
    while (
      (!observedBody ||
        ["sending", "running", "waiting"].includes(p.snapshot(input).status)) &&
      Date.now() < skillDeadline
    )
      await new Promise((r) => setTimeout(r, 100));
    assert.match(
      JSON.stringify(observedBody.messages),
      /SYNTHETIC_SKILL_SENTINEL/,
    );
    ...
    assert.ok(
      p
        .snapshot(input)
        .items.some(
          (item) => item.text === "/synthetic-command Synthetic user task",
        ),
    );
    observedBody = undefined;
    const png =
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a6YQAAAAASUVORK5CYII=";
    const runtime = p.get(input).runtime;
    const rpc = (method, params) =>
      p.transport("default").rpc("default", method, params);
    const image = await rpc("image.attach_bytes", {
      session_id: runtime,
      filename: "synthetic.png",
      content_base64: png,
    });
    assert.equal(image.attached, true);
    const fileImage = await files("read", {
      agentId: "default",
      path: image.path,
    });
    ...
    assert.equal(
      (await rpc("image.detach", { session_id: runtime, path: image.path }))
        .detached,
      true,
    );
    await p.send({
      ...
      attachments: [
        { name: "synthetic.png", data: png },
        {
          name: "synthetic-note.txt",
          data: Buffer.from("SUSHIAI_ATTACHMENT_SENTINEL").toString("base64"),
        },
      ],
    });
    const deadline = Date.now() + 30000;
    while (!observedBody && Date.now() < deadline)
      await new Promise((r) => setTimeout(r, 100));
    assert.ok(observedBody, "model must receive the prompt");
    assert.match(
      JSON.stringify(observedBody.messages),
      /SUSHIAI_ATTACHMENT_SENTINEL|@file:/,
    );
    while (
      ["sending", "running", "waiting"].includes(p.snapshot(input).status) &&
      Date.now() < deadline
    ...
    const live = await p.operations.get("conversations.media")({
      ...input,
      itemId: liveImage.id,
    });
    ...
    assert.equal(
      (
        await p.operations.get("conversations.media")({
          ...input,
          itemId: savedImage.id,
        })
      ).dataUrl,
      live.dataUrl,
    );
    ...
    assert.equal(
      await fs.readFile(home + "/attachments/synthetic-note.txt", "utf8"),
      "SUSHIAI_ATTACHMENT_SENTINEL",
    );
    console.log(
      JSON.stringify({
        passed: true,
        nativeFileStaging: true,
        nativeImageStagingAndDetach: true,
        modelReceivedReference: true,
        nativeImagePreviewAfterRestart: true,
        nativeFileEditor: true,
        staleFileSaveRejected: true,
        nativeGitReview: true,
        encodingPreserved: true,
        nativeFileImagePreview: true,
        nativeSkillCommand: true,
      }),
    );
  } finally {
    await p.close();
