...
const {
  ...
  validateImageData,
  installHermesMedia,
} = require("../electron/agents/hermes-media.cjs");
...
test("native persisted image directives retain order, quoted paths and caption", () => {
  ...
  assert.deepEqual(
    rows.filter((x) => x.kind === "image").map((x) => x.path),
    ["/synthetic/my photo.png", "/synthetic/second.png"],
  );
  assert.equal(rows[0].text, "Caption\n");
  assert.equal(
    splitImageReferences("Example @image:/not-a-directive.png")[0].kind,
    "text",
  );
  assert.equal(
    splitImageReferences("@image:https://example.com/image.png")[0].kind,
    "text",
  );
  assert.equal(
    normalizeHistory([
      { role: "assistant", content: "@image:/synthetic/test.png" },
    ])[0].kind,
    "text",
  );
});
test("media reads resolve item IDs in the bound conversation, never renderer paths", async () => {
  ...
  const provider = {
    ...
    operations: new Map(),
    get(input) {
      if (input.agentId !== "a" || input.conversationId !== "chat")
        throw Error("Unknown conversation");
      return {
        agentId: "a",
        state: {
          items: [
    ...
    transport(agent) {
      ...
      return {
        async request(...args) {
          calls.push(args);
          return { data_url: "data:image/png;base64,YQ==" };
        },
      };
    },
  };
  ...
  assert.deepEqual(calls[0], [
    "GET",
    "/api/media",
    { profile: "a", query: { path: "/synthetic/photo.png" } },
  ]);
});
test("preview only accepts bounded canonical raster data URLs", () => {
  for (const data of [
    ...
    assert.throws(() => validateImageData(data));
});
