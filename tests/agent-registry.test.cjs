...
test("providers use the same contract without exposing transport access", async () => {
  ...
  const make = (id) => ({
    descriptor: { apiVersion: 1, id, capabilities: ["conversations"] },
    operations: new Map([
      ["agents.list", async () => [{ id: "one", providerId: id }]],
    ]),
    close: async () => {},
  });
  ...
  assert.equal(
    (await registry.call("alternative", "agents.list"))[0].providerId,
    "alternative",
  );
  await assert.rejects(
    registry.call("hermes", "rpc", { method: "anything" }),
    /not supported/,
  );
  await assert.rejects(
    registry.call("missing", "agents.list"),
    /not installed/,
  );
...
test("provider event identity cannot spoof another provider; closing releases resources", async () => {
  ...
  let closed = 0;
  const provider = {
    descriptor: { apiVersion: 1, id: "one" },
    operations: new Map(),
    close: () => closed++,
  };
