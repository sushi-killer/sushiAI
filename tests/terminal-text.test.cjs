...
// This app targets macOS, where /bin/zsh is the default shell. CI's ubuntu
// runner (only ever exercised once "npm test" started running the full
// suite, not a hand-picked subset) has neither the binary at this path nor
// the locales this test depends on - skip rather than fail there.
...
test(
  "GUI and ASCII locale launches enable multibyte zsh editing",
  {
    skip: HAS_ZSH ? false : "zsh not available on this runner",
  },
  () => {
    for (const env of [
      {},
