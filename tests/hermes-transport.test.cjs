...
const {
  HermesTransport,
  LIMITS,
} = require("../electron/agents/hermes-transport.cjs");

const flush = async () => {
  for (let i = 0; i < 12; i++) await Promise.resolve();
};
const wireEvent = (socket, type, payload = {}, extra = {}) =>
  socket.emit(
    "message",
    JSON.stringify({
      jsonrpc: "2.0",
      method: "event",
      params: { type, payload, ...extra },
    }),
  );
class Socket extends EventTarget {
  ...
  reply(index, result) {
    this.emit(
      "message",
      JSON.stringify({ jsonrpc: "2.0", id: this.sent[index].id, result }),
    );
  }
}
function fixture(t, options = {}) {
  ...
  return {
    ...
    ready: async () => {
      ...
      children[0].stdout.emit("data", "HERMES_BACKEND_READY port=32123\n");
      await p;
    },
  };
}
...
test("reconnect replays missing events before held live deltas without resending prompts", async (t) => {
  const f = fixture(t);
  await f.ready();
  const first = f.transport.rpc("default", "session.create", {});
  await flush();
  const a = f.sockets[0];
  a.emit("open");
  await flush();
  wireEvent(a, "gateway.ready", { replay_epoch: "epoch-one" });
  a.reply(0, { session_id: "s" });
  await first;
  wireEvent(a, "message.delta", { text: "one" }, { session_id: "s", seq: 1 });
  a.emit("close");
  const second = f.transport.rpc("default", "gateway.ping", {});
  await flush();
  const b = f.sockets[1];
  b.emit("open");
  await flush();
  wireEvent(b, "gateway.ready", { replay_epoch: "epoch-one" });
  await flush();
  const replay = b.sent.findIndex((x) => x.method === "session.events.since");
  ...
  await flush();
  b.reply(
    b.sent.findIndex((x) => x.method === "gateway.ping"),
    { ok: true },
  );
  await second;
  assert.deepEqual(
  ...
  assert.equal(
    b.sent.some((r) => r.method === "session.create"),
    false,
  );
...
test("single-flight native startup, split sentinel, owned token and isolated environment", async (t) => {
  const f = fixture(t, {
    env: {
      HERMES_HOME: "/fake/hermes-home",
      HERMES_DASHBOARD_SESSION_TOKEN: "not-owned",
      HERMES_DESKTOP_READY_FILE: "/never/write",
      HERMES_PARENT_NONCE: "stale",
    },
  });
  const p = f.transport.start();
  assert.equal(f.transport.start(), p);
...
test("dedicated backend pins all RPCs to its profile", async (t) => {
  ...
  await assert.rejects(f.transport.rpc("other", "config.get"), {
    code: "PROFILE_MISMATCH",
  });
  assert.equal(f.sockets.length, 0);
  const p = f.transport.rpc("work", "session.create", { profile: "other" });
  await flush();
  const s = f.sockets[0];
  ...
  assert.equal(
    ...
    f.calls[0][2].env.HERMES_DASHBOARD_SESSION_TOKEN,
  );
  s.emit("open");
  await flush();
  assert.equal(s.sent[0].params.profile, "work");
  s.reply(0, { session_id: "s1" });
  assert.deepEqual(await p, { session_id: "s1" });
  s.emit(
    "message",
    JSON.stringify({
      jsonrpc: "2.0",
      method: "event",
      params: { type: "token", seq: 7, profile: "wrong" },
    }),
  );
...
test("HTTP authenticated JSON, canonical profile query, explicit scope and body", async (t) => {
  ...
  await f.ready();
  assert.deepEqual(
    await f.transport.request("POST", "/api/config?profile=work&profile=work", {
      profile: "work",
      query: { profile: "work", q: "a & b", count: 2 },
      body: { profile: "work", x: 1 },
    }),
    { ok: true },
  );
  const [raw, opts] = f.fetches[0],
    url = new URL(raw);
  ...
  assert.equal(
    opts.headers["X-Hermes-Session-Token"],
    f.calls[0][2].env.HERMES_DASHBOARD_SESSION_TOKEN,
  );
  assert.deepEqual(JSON.parse(opts.body), { profile: "work", x: 1 });
  await assert.rejects(
    f.transport.request("GET", "/api/status", { profile: "another" }),
    /PROFILE_MISMATCH/,
  );
...
test("invalid URLs and oversized requests never launch or fetch", async (t) => {
  ...
  await assert.rejects(
    f.transport.request("POST", "/api/x", {
      body: "x".repeat(LIMITS.messageBytes),
    }),
    { code: "MESSAGE_TOO_LARGE" },
  );
  await assert.rejects(
    f.transport.rpc("default", "session.create", {
      huge: "x".repeat(LIMITS.messageBytes),
    }),
    { code: "MESSAGE_TOO_LARGE" },
  );
...
test("HTTP bounds streamed bodies and rejects malformed JSON and redirects", async (t) => {
  const responses = [
    ...
    new Response("x", {
      headers: { "content-length": String(LIMITS.messageBytes + 1) },
    }),
    new Response(
      new ReadableStream({
        start(c) {
          c.enqueue(new Uint8Array(LIMITS.messageBytes));
  ...
  const f = fixture(t, { fetcher: async () => responses.shift() });
  await f.ready();
  for (const code of [
    "INVALID_JSON",
    "REDIRECT_FORBIDDEN",
...
test("RPC IDs correlate out-of-order replies, propagate codes, and share connection", async (t) => {
  const f = fixture(t);
  await f.ready();
  const a = f.transport.rpc("default", "one"),
    b = f.transport.rpc("default", "two");
  await flush();
  ...
  const p = f.transport.rpc("default", "bad");
  await flush();
...
test("disconnect rejects pending RPC without resend; next call reconnects", async (t) => {
  const f = fixture(t);
  await f.ready();
  const p = f.transport.rpc("default", "session.create");
  await flush();
  f.sockets[0].emit("open");
  await flush();
  const check = assert.rejects(p, { code: "WS_CLOSED" });
  f.sockets[0].emit("close");
  await check;
  const next = f.transport.rpc("default", "session.events.since", {
    session_id: "s",
    last_seen: 7,
  });
  await flush();
  ...
  assert.equal(f.sockets[1].sent[0].method, "session.events.since");
  f.sockets[1].reply(0, { events: [] });
  await next;
});
...
test("malformed, binary, oversized WS frames fail pending calls", async (t) => {
  for (const data of [
    ...
    "[]",
    new Uint8Array(1),
    "x".repeat(LIMITS.messageBytes + 1),
  ]) {
    const f = fixture(t);
    await f.ready();
    const p = f.transport.rpc("default", "ping");
    await flush();
    f.sockets[0].emit("open");
    await flush();
    const check = assert.rejects(p);
    f.sockets[0].emit("message", data);
    await check;
...
test("close cancels HTTP even when fetch ignores abort; owned child only and idempotent", async (t) => {
  ...
  await f.ready();
  const p = f.transport.request("GET", "/api/status");
  await flush();
...
test("invalid port and unbounded startup line fail safely", async (t) => {
  for (const [data, code] of [
    ["HERMES_BACKEND_READY port=65536\n", "INVALID_PORT"],
    ["x".repeat(LIMITS.lineBytes + 1), "STARTUP_OUTPUT_TOO_LARGE"],
  ]) {
    const f = fixture(t);
    const check = assert.rejects(f.transport.start(), { code });
    f.children[0].stdout.emit("data", data);
    await check;
  }
});
...
test("bounded admission during startup", async (t) => {
  const f = fixture(t);
  const pending = Array.from({ length: LIMITS.pending }, () =>
    f.transport.request("GET", "/api/status"),
  );
  const checks = pending.map((p) => assert.rejects(p, { code: "CLOSED" }));
  await assert.rejects(f.transport.request("GET", "/api/status"), {
...
test("timeouts cover startup, connecting WS, and unresponsive HTTP", async (t) => {
  ...
  const f = fixture(t);
  const check = assert.rejects(f.transport.start(), {
    code: "STARTUP_TIMEOUT",
  });
  t.mock.timers.tick(LIMITS.startupMs);
  await check;
  const g = fixture(t);
  await g.ready();
  const rpc = g.transport.rpc("default", "ping");
  await flush();
  const rpcCheck = assert.rejects(rpc, { code: "WS_CONNECT_TIMEOUT" });
  t.mock.timers.tick(LIMITS.connectMs);
  await rpcCheck;
  const h = fixture(t, { fetcher: () => new Promise(() => {}) });
  await h.ready();
  const req = h.transport.request("GET", "/api/status");
  await flush();
...
test("minted token never appears in returned payloads or callbacks", async (t) => {
  let token;
  const f = fixture(t, {
    fetcher: async () => new Response(JSON.stringify({ echoed: token })),
  });
  await f.ready();
  token = f.calls[0][2].env.HERMES_DASHBOARD_SESSION_TOKEN;
  assert.deepEqual(await f.transport.request("GET", "/api/status"), {
    echoed: "[redacted]",
  });
  const p = f.transport.rpc("default", "ping");
  await flush();
  f.sockets[0].emit("open");
  await flush();
  f.sockets[0].reply(0, token);
  assert.equal(await p, "[redacted]");
  const escaped = token
    .split("")
  ...
  f.sockets[0].emit(
    "message",
    `{"jsonrpc":"2.0","method":"event","params":{"type":"${escaped}"}}`,
  );
...
test("ready child failure cancels active work and closes its socket", async (t) => {
  ...
  f.children[0].emit("error", new Error("private diagnostic"));
  await check;
  assert.equal(f.sockets[0].closes, 1);
  await assert.rejects(f.transport.rpc("default", "ping"), {
    code: "CHILD_ERROR",
  });
});
...
test("RPC timeout releases slot and ignores late replies", async (t) => {
  ...
  f.sockets[0].emit("open");
  await flush();
  const check = assert.rejects(p, { code: "REQUEST_TIMEOUT" });
  t.mock.timers.tick(LIMITS.requestMs);
  await check;
  f.sockets[0].reply(0, "too late");
  const next = f.transport.rpc("default", "ping");
  await flush();
  f.sockets[0].reply(1, "ok");
...
test("backpressure rejects instead of buffering; observer exceptions are isolated", async (t) => {
  ...
  const check = assert.rejects(p, { code: "WS_SEND_FAILED" });
  f.sockets[0].emit("open");
  await check;
  assert.equal(f.sockets[0].sent.length, 0);
});
...
test("owned child gets bounded SIGTERM to SIGKILL escalation", async (t) => {
  ...
  assert.deepEqual(f.children[0].kills, ["SIGTERM"]);
  t.mock.timers.tick(LIMITS.killMs);
  assert.deepEqual(f.children[0].kills, ["SIGTERM", "SIGKILL"]);
  f.children[0].signalCode = "SIGKILL";
...
test("spawn and fetch exceptions cannot leak native error messages", async (t) => {
  const f = fixture(t, {
    spawn: () => {
      throw new Error("sensitive spawn environment");
    },
  });
  await assert.rejects(f.transport.start(), { code: "CHILD_ERROR" });
  const g = fixture(t, {
    fetcher: () => {
      throw new Error("Hermes transport: secret-token");
    },
  });
  await g.ready();
  await assert.rejects(g.transport.request("GET", "/api/status"), {
...
test("long native actions have an explicit bounded deadline", async (t) => {
  t.mock.timers.enable({ apis: ["setTimeout"] });
  const f = fixture(t, { fetcher: () => new Promise(() => {}) });
  await f.ready();
  let settled = false;
  const request = f.transport.request("POST", "/api/cron/jobs/test/trigger", {
    timeoutMs: 60000,
  });
  request.then(
    () => {
  ...
  const check = assert.rejects(request, { code: "REQUEST_TIMEOUT" });
  await flush();
  t.mock.timers.tick(LIMITS.requestMs);
  await flush();
  ...
  await check;
  await assert.rejects(
    f.transport.request("GET", "/api/status", { timeoutMs: 600001 }),
