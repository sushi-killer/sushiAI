...
test("file saves use native scoped methods and reject stale edits and incomplete previews", async () => {
  ...
  const call = (action, data = {}) =>
    p.operations.get("addons.files." + action)({
      agentId: "a",
      path: "/synthetic/a.txt",
      ...data,
    });
  assert.equal((await call("list", { path: undefined })).path, "/synthetic");
  const opened = await call("read");
  content = "External change";
  await assert.rejects(
    call("save", { revision: opened.revision, content: "Overwrite" }),
    /changed on disk/,
  );
  assert.equal(calls.filter((c) => c.method === "POST").length, 0);
  const fresh = await call("read");
  assert.equal(
    (await call("save", { revision: fresh.revision, content: "Saved" })).text,
    "Saved",
  );
  truncated = true;
  await assert.rejects(
    call("save", { revision: fresh.revision, content: "No" }),
    /complete text/,
  );
...
test("lossy native text previews cannot overwrite original bytes, while literal replacement characters remain editable", async () => {
    ...
    transport() {
      return {
        async request(method, route, options) {
          ...
          if (route.endsWith("read-data-url"))
            return {
              dataUrl: "data:text/plain;base64," + bytes.toString("base64"),
            };
          if (route.endsWith("write-text")) {
            writes++;
            bytes = Buffer.from(options.body.content);
            return { ok: true };
          }
        },
      };
  ...
  const lossy = await call("read");
  assert.match(lossy.readOnlyReason, /not lossless UTF-8/);
  await assert.rejects(
    call("save", { revision: lossy.revision, content: "Changed" }),
    /not lossless/,
  );
  assert.equal(writes, 0);
  assert.deepEqual(bytes, Buffer.from([0xff]));
  bytes = Buffer.from("Valid \uFFFD character", "utf8");
  const valid = await call("read");
  assert.equal(valid.readOnlyReason, "");
  assert.equal(
    (
      await call("save", {
...
test("raster file previews use native byte reads and remain non-editable", async () => {
  ...
  assert.equal(image.imageData, "data:image/png;base64,YQ==");
  assert.equal(image.byteSize, 1);
  assert.equal(image.binary, true);
  assert.deepEqual(calls[0], {
    method: "GET",
    route: "/api/fs/read-data-url",
    options: { profile: "a", query: { path: input.path } },
  });
  await assert.rejects(
    p.operations.get("addons.files.save")({
      ...input,
      revision: "forged",
      content: "Overwrite",
    }),
