...
function validateAttachments(value) {
  ...
  if (!Array.isArray(value) || value.length > 8)
    throw Error("Attach up to 8 files.");
...
async function stageAttachments(rpc, runtime, files) {
  ...
  const detach = async () => {
    const results = await Promise.allSettled(
      images.map((path) => rpc("image.detach", { session_id: runtime, path })),
    );
    if (results.some((r) => r.status === "rejected"))
      throw Error(
        "An image could not be detached. Reopen this conversation before retrying.",
      );
  };
  try {
    for (const file of files) {
      const result = await rpc(
        file.image ? "image.attach_bytes" : "file.attach",
        {
          session_id: runtime,
          ...(file.image
            ? { filename: file.name, content_base64: file.data }
            : {
                name: file.name,
                data_url: `data:application/octet-stream;base64,${file.data}`,
              }),
        },
      );
      ...
      if (file.image) {
        ...
        media.push({ path: result.path, name: file.name });
      } else if (
        typeof result.ref_text !== "string" ||
        !result.ref_text.startsWith("@file:")
      )
        throw Error(`Hermes did not return a file reference for ${file.name}.`);
      refs.push(
        file.image ? `[Attached image: ${file.name}]` : result.ref_text,
      );
    }
