...
export const useAttachmentDrafts = () =>
...
export async function addAttachmentFiles(id: string, files: File[]) {
  ...
  if (state.loading[id]) {
    setAttachmentError(id, "Wait for the current file read to finish.");
    return;
  }
  if ((state.files[id]?.length || 0) + files.length > 8) {
    setAttachmentError(id, "Attach up to 8 files per message.");
    return;
  }
  update({
    ...state,
  ...
  try {
    ...
    update({ ...state, loading: { ...state.loading, [id]: false } });
  }
}
