...
test("Git reads resolve project root, pin agent scope and reject escaping file paths", async () => {
    ...
    transport() {
      return {
        async request(method, route, options) {
          ...
          if (route.endsWith("default-cwd"))
            return { cwd: "/synthetic/repo/nested" };
          if (route.endsWith("git-root")) return { root };
          if (route.endsWith("/status"))
            return { branch: "main", ahead: 0, behind: 0 };
          if (route.endsWith("/list"))
            return {
              files: [{ path: "note.txt", status: "M", added: 1, removed: 1 }],
            };
  ...
  const call = (action, input = {}) =>
    p.operations.get("addons.git." + action)({ agentId: "a", ...input });
  ...
  for (const file of [
    ...
    await assert.rejects(call("diff", { file }), /relative/);
  root = null;
  assert.equal((await call("status")).repository, false);
  await assert.rejects(
    call("diff", { file: "note.txt" }),
