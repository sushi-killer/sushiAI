...
function installHermesGit(provider) {
  ...
  for (const action of ["status", "diff"])
    provider.operations.set(`addons.git.${action}`, async (input) => {
      if (
        ...
        throw Error("Unknown Git request field.");
      const agent = await provider.agent(input);
      const request = (route, query) =>
        provider
          .transport(agent)
          .request("GET", route, { profile: agent, query });
      const target = text(
        input.path || (await request("/api/fs/default-cwd")).cwd,
        "project path",
        4096,
      );
      if (!path.isAbsolute(target))
        throw Error("Enter an absolute project path.");
      const root = (await request("/api/fs/git-root", { path: target })).root;
      if (!root) {
        if (action === "status")
          return { path: target, repository: false, files: [] };
        throw Error("This folder is not in a Git repository.");
      }
      if (action === "status") {
        const status = await request("/api/git/status", { path: root });
        if (!status) throw Error("Could not read Git status.");
        const review = await request("/api/git/review/list", {
          path: root,
          scope: "uncommitted",
        });
        ...
        return {
          ...
          files: review.files.slice(0, 2000),
          truncated: review.files.length > 2000,
        };
      }
