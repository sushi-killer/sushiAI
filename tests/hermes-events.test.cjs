...
const {
  normalizeHistory,
  reduceEvent,
} = require("../electron/agents/hermes-events.cjs");
...
test("projected RPC history keeps supplied fields without inventing missing results", () => {
  ...
  assert.deepEqual(
    normalizeHistory([
      { role: "tool", name: "terminal", args: {}, result: "ok" },
    ])[0].output,
    "ok",
  );
});
...
test("structured content preserves block order, reasoning and tool results", () => {
  const items = normalizeHistory([
    ...
    {
      role: "user",
      content: [{ type: "tool_result", tool_use_id: "a", content: "/example" }],
    },
  ]);
  assert.deepEqual(
    items.map((i) => i.kind),
...
test("reasoning-only history and real summaries survive; encrypted data does not", () => {
  const items = normalizeHistory([
    {
      role: "assistant",
      reasoning_details: [{ type: "reasoning.text", text: "Actual thought." }],
    },
    {
      role: "assistant",
  ...
  assert.deepEqual(
    items.map((i) => i.text),
    ["Actual thought.", "Actual summary.", "Same"],
  );
});
...
test("malformed arguments and orphan results are retained", () => {
  const items = normalizeHistory([
    {
      role: "assistant",
      tool_calls: [
        { id: "bad", function: { name: "terminal", arguments: "{oops" } },
      ],
    },
    {
      role: "tool",
  ...
  assert.equal(items[0].input, "{oops");
  assert.equal(items[1].output, "output");
});
...
test("null and falsy tool results remain present, including orphan failures", () => {
  for (const result of [null, false, 0, ""]) {
    ...
    assert.equal(
      run(event("tool.complete", { tool_id: "a", result })).items[0].output,
      result,
    );
  }
  assert.equal(
    normalizeHistory([{ role: "tool", content: '{"success":false}' }])[0]
      .status,
    "error",
  );
});
...
test("categories use exact tools/paths, never descriptive prose", () => {
  ...
  const items = normalizeHistory(
    names.map((name) => ({
      role: "tool",
      name,
      context: "updated memory and skills",
    })),
  );
  assert.deepEqual(
    items.map((i) => i.category),
    ["memory", "skills", "skills", "skills", "terminal", "other"],
  );
  assert.equal(
    normalizeHistory([
      { role: "tool", name: "read_file", args: { path: "/example/SKILL.md" } },
    ])[0].category,
    "skills",
  );
});
...
test("native JSON-RPC envelopes and Desktop bare events normalize identically", () => {
  const e = event(
    "message.delta",
    { text: "Hi" },
    { seq: 1, session_id: "synthetic" },
  );
  assert.deepEqual(
    reduceEvent(undefined, e),
    reduceEvent(undefined, { jsonrpc: "2.0", method: "event", params: e }),
  );
});
...
test("streamed interims, tools and final text keep order without duplicate bubbles", () => {
  ...
  assert.equal(s.items[2].output, "/example");
...
test("interim arriving after tool start seals its earlier streamed text", () => {
  const s = run(
    ...
    event("message.delta", { text: "Final" }),
    event("message.complete", { text: "Final" }),
  );
  assert.deepEqual(s.items.map((i) => i.text).filter(Boolean), [
    "Checking.",
    "Final",
  ]);
});
...
test("replayed sequence is idempotent through serialized snapshots, repeated tokens are not deduped", () => {
  let s = run(
    start(),
    event("message.delta", { text: "ha" }, { seq: 1, session_id: "s" }),
  );
  s = reduceEvent(
  ...
  s = reduceEvent(
    s,
    event("message.delta", { text: "ha" }, { seq: 2, session_id: "s" }),
  );
...
test("parallel same-name tools correlate by ID and preserve args through progress", () => {
  ...
  assert.equal(s.items[0].input.command, "one");
  assert.equal(s.items[1].status, "error");
  assert.equal(s.items[0].status, "complete");
});

test("thinking/status/MoA progress do not synthesize reasoning", () => {
  const s = run(
    event("thinking.delta", { text: "Thinking…" }),
...
for (const kind of ["approval", "clarify", "sudo", "secret"]) {
  test(`${kind} cards retain request ID/kind, replay safely and expire by ID`, () => {
    ...
    let s = run(
      start(),
      event(`${kind}.request`, payload),
      event(`${kind}.request`, payload),
    );
    assert.equal(s.requests.length, 1);
    assert.equal(s.requests[0].id, "req");
    assert.equal(s.requests[0].kind, kind);
...
test("batch clarify data and simultaneous request kinds survive", () => {
  ...
  assert.equal(s.requests[0].input.answers.q, "A");
});
...
test("completion merges usage, clears requests, and preserves partial errors", () => {
  ...
  assert.equal(s.items[0].text, "Partial");
  assert.equal(s.items[1].kind, "notice");
...
test("cancel ends pending work and late deltas do not revive it", () => {
  const s = run(
    start(),
    event("tool.start", { tool_id: "t", name: "memory" }),
    event("secret.request", { request_id: "r" }),
    event("message.complete", { status: "interrupted", text: "" }),
    event("message.delta", { text: "late" }),
  );
  assert.equal(s.status, "cancelled");
  assert.equal(s.items[0].status, "cancelled");
...
test("session info, title, and native session.usage update live values", () => {
  const s = run(
    ...
    event("session.usage", { usage: { output: 5 } }),
    event("session.title", { title: "Example" }),
    event("session.info", { running: false }),
  );
  assert.equal(s.info.model, "example-model");
  assert.equal(s.info.title, "Example");
...
test("memory/skill tool updates and review summaries retain meaningful data", () => {
  const s = run(
    event("tool.complete", {
      tool_id: "m",
      name: "memory",
      args: { action: "add", target: "user", content: "Example preference" },
      result: { success: true },
    }),
    event("tool.complete", {
      tool_id: "k",
  ...
  assert.equal(s.items[0].category, "memory");
  assert.equal(s.items[1].category, "skills");
...
test("subagent identity and output remain separate from the parent lifecycle", () => {
  ...
  assert.equal(s.items[1].output.tool_name, "terminal");
  assert.equal(s.items[2].output.subagent_id, "child");
});
...
test("todo revisions, keyed notices, and unknown events are retained predictably", () => {
  const s = run(
    event("todo.updated", {
      revision: 2,
      todos: [{ id: "a", content: "Example", status: "pending" }],
    }),
    ...
    event("notification.clear", { key: "credits" }),
    event("future.event", { nested: { count: 1 } }),
  );
  assert.equal(s.items[0].output.length, 1);
  assert.equal(
    s.items.some((i) => i.kind === "notice"),
    false,
  );
  assert.equal(s.items.at(-1).name, "future.event");
  assert.equal(s.items.at(-1).output.nested.count, 1);
});
...
test("states and incoming nested payloads are independent immutable snapshots", () => {
  ...
  const after = reduceEvent(
    before,
    event("tool.complete", { tool_id: "t", result: { ok: true } }),
  );
  ...
  assert.equal(e.payload.args.nested.command, "example");
  assert.deepEqual(
    reduceEvent(JSON.parse(saved), event("session.title", { title: "x" })),
...
test("defensive bounds, cycles, prototype keys and HTML remain plain safe data", () => {
  ...
  assert.ok(
    normalizeHistory(
      Array.from({ length: 2100 }, () => ({ role: "user", text: "x" })),
    ).length <= 2000,
  );
  assert.ok(
    run(event("message.delta", { text: "x".repeat(100000) })).items[0].text
      .length <= 65536,
  );
  const polluted = reduceEvent(
    undefined,
    event("session.info", JSON.parse('{"__proto__":{"polluted":true}}')),
  );
  assert.equal(polluted.info.polluted, undefined);
  assert.equal({}.polluted, undefined);
});

test("successful history hydration updates state without polluting the transcript", () => {
  const s = run(
    event("session.resume_progress", { phase: "history", status: "loading" }),
    event("session.resume_progress", { phase: "history", status: "complete" }),
  );
  assert.equal(s.items.length, 0);
  assert.equal(s.info.hydrating, false);
